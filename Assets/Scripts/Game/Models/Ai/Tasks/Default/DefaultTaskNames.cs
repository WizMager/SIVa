namespace Game.Models.Ai.Tasks.Default
{
	public static class DefaultTaskNames
	{
		public const string SEQUENCE = "sequence";
		public const string PARALLEL = "parallel";
		public const string ASYNCPARALLEL = "async parallel";
		public const string SELECTOR = "selector";
		public const string SELECTOR_RANDOM = "selector random";
		public const string INVERTER = "inverter";
		public const string REPEATER = "repeater";
		public const string REPEATER_UNTIL_FAILURE = "repeatUntilFailure";
		public const string RETURN_FAILURE = "return failure";
		public const string RETURN_SUCCESS = "return success";
		public const string SELECTOR_REPEATER = "selector repeater";
		
		public const string SUCCESS_WITH_CHANCE = "success with chance";
		public const string SUCCESS_WITH_CHANCE_PATH = "Utils/success with chance";
	}
}